...
let  prestat= document.querySelector(".pressure-status");
...
function getWeatherData(city, unit, hourlyorWeek) {
  fetch(
    `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${city}?unitGroup=metric&key=39LBA7SXS7XUBT6QKTTEC2C9W&contentType=json`,
    {
      method: "GET",
      headers: {},
    }
...
function updatepressureStatus(pressure) {
  // Customize the pressure ranges based on your preferences
  console.log(pressure)
  if (pressure <= 1000) {
      prestat.innerText = "Low Pressure";
  } else if (pressure <= 1015) {
      prestat.innerText = "Normal Pressure";
  } else {
      prestat.innerText = "High Pressure";
  }
}
...
// Assuming searchForm and search variables are defined elsewhere
searchForm.addEventListener("submit", (e) => {
  e.preventDefault();
  let location = search.value;
  if (location) {
    currentCity = location;
    getWeatherDatanew(location);
  }
});

function getWeatherDatanew(location) {
  const BaseURL = 'https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/weatherdata/';
  const ApiKey = '39LBA7SXS7XUBT6QKTTEC2C9W';
  const UnitGroup = 'metric';
  const QueryType = 'FORECAST';
  const AggregateHours = '1';
  const url = `${BaseURL}${QueryType.toLowerCase()}?aggregateHours=${AggregateHours}&unitGroup=${UnitGroup}&locations=${location}&key=${ApiKey}&contentType=json`;

  fetch( `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${location}?unitGroup=metric&key=39LBA7SXS7XUBT6QKTTEC2C9W&contentType=json`)
    .then(response => response.json())
    .then(data => {
      weatherDataList.push(data);
      console.log("Weather data:", data);
      sendWeatherDataToServer(location);
    })
    .catch(error => {
...
function sendWeatherDataToServer(location) {
  if (weatherDataList.length === 0) {
    console.error('No weather data to send to server');
    return;
  }
  console.log("weatherDataList before sending:", weatherDataList); // Log for debugging
  fetch(`http://127.0.0.1:3001/save-weather-data?location=${location}`, {
      method: 'POST',
      headers: {
  ...
  .then(response => response.json())
  .then(data => {
      console.log('Weather data saved on server:', data);
  })
  .catch(error => {
      console.error('Error saving weather data on server:', error);
  });
}
